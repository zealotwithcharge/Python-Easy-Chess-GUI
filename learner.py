...
class EasyChessGui:
    ...
    def define_timer(self, window, name='human'):
        ...
        """
        if name == 'human':
            timer = Timer(self.human_tc_type, 5000000000000000000000,
                          self.human_inc_time_ms, self.human_period_moves)           
        else:
            timer = Timer(self.engine_tc_type, 50000000000000000000000000,
    ...
    def play_game(self, window, engine_id_name, board, move21 = None):
        """
        ...
        if args.color == "white":
            is_human_stm = True
        elif args.color == 'both':
            is_human_stm = random.choice([True,False])
            pass
        else:
            is_human_stm = False
        move_state = 0
                    ...
                    if type(button) is tuple:
                        ...
                        # Else if to_sq button is pressed
                            ...
                            if move_to == move_from:
                                ...
                                move_state = 0
                                continue
                            piece1 = self.psg_board[to_row][to_col]  # get the move-from piece
                            
                            # Change the color of the "fr" board square
                            if (args.color == 'white' and piece1 in range(7,13)) or (args.color == 'black' and piece1 in range(1,7)) :
                                fr_row, fr_col = move_from
                                button_square.Update(button_color=('white', self.sq_dark_color))
                                self.change_square_color(window, to_row, to_col)
                                piece = self.psg_board[to_row][to_col]
                                move_from = move_to
                                move_state = 1
                                moved_piece = board.piece_type_at(chess.square(to_col, to_row)) 
                                continue
        ...
        sg.Popup('Good Job!', title=BOX_TITLE,
                     icon=ico_path[platform]['pecg'])
